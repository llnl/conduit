...
#else
#  define FMT_ENABLE_IF(...) conduit_fmt::enable_if_t<(__VA_ARGS__), int> = 0
#endif
...
#  define FMT_ASSERT(condition, message) \
    conduit_fmt::detail::ignore_unused((condition), (message))
#else
#  define FMT_ASSERT(condition, message)                                    \
    ((condition) /* void() fails with -Winvalid-constexpr on clang 4.0.1 */ \
         ? (void)0                                                          \
         : ::conduit_fmt::assert_fail(__FILE__, __LINE__, (message)))
#endif
...
class basic_specs {
 ...
 public:
  ...
  constexpr auto align() const -> align {
    return static_cast<conduit_fmt::align>((data_ & align_mask) >> align_shift);
  }
  FMT_CONSTEXPR void set_align(conduit_fmt::align a) {
  ...
  constexpr auto sign() const -> sign {
    return static_cast<conduit_fmt::sign>((data_ & sign_mask) >> sign_shift);
  }
  FMT_CONSTEXPR void set_sign(conduit_fmt::sign s) {
...
FMT_INLINE void println(format_string<T...> fmt, T&&... args) {
  return conduit_fmt::println(stdout, fmt, static_cast<T&&>(args)...);
}
