...
#  define FMT_COMPILE(s) FMT_STRING_IMPL(s, conduit_fmt::compiled_string)
#else
...
template <typename Char, typename V, int N> struct spec_field {
  ...
  constexpr FMT_INLINE auto format(OutputIt out, const T&... args) const
      -> OutputIt {
    const auto& vargs =
        conduit_fmt::make_format_args<basic_format_context<OutputIt, Char>>(args...);
...
constexpr auto parse_specs(basic_string_view<Char> str, size_t pos,
  ...
  return {f, pos + conduit_fmt::detail::to_unsigned(end - str.data()),
...
template <typename T, typename Args, size_t END_POS, int ARG_INDEX, int NEXT_ID,
          typename S>
constexpr auto parse_replacement_field_then_tail(S conduit_fmt) {
...
FMT_INLINE FMT_CONSTEXPR_STRING auto format(const S&, T&&... args)
  ...
  if constexpr (std::is_same<typename S::char_type, char>::value) {
    ...
    if constexpr (str.size() == 2 && str[0] == '{' && str[1] == '}') {
      ...
      if constexpr (detail::is_named_arg<
        ...
        return conduit_fmt::to_string(first.value);
      } else {
        return conduit_fmt::to_string(first);
      }
    }
  }
  ...
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
    ...
    return conduit_fmt::format(
        static_cast<basic_string_view<typename S::char_type>>(S()),
        std::forward<T>(args)...);
  } else {
    return conduit_fmt::format(compiled, std::forward<T>(args)...);
  }
}
...
FMT_CONSTEXPR auto format_to(OutputIt out, const S&, T&&... args) -> OutputIt {
  ...
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
                             detail::unknown_format>()) {
    return conduit_fmt::format_to(
        out, static_cast<basic_string_view<typename S::char_type>>(S()),
        std::forward<T>(args)...);
  } else {
    return conduit_fmt::format_to(out, compiled, std::forward<T>(args)...);
  }
}
#endif
...
auto format_to_n(OutputIt out, size_t n, const S& fmt, T&&... args)
  ...
  conduit_fmt::format_to(std::back_inserter(buf), fmt, std::forward<T>(args)...);
...
FMT_CONSTEXPR20 auto formatted_size(const S& fmt, T&&... args) -> size_t {
  ...
  conduit_fmt::format_to(appender(buf), fmt, std::forward<T>(args)...);
  return buf.count();
}
...
template <size_t N> class static_format_result {
 ...
 public:
  ...
  auto str() const -> conduit_fmt::string_view { return {data, N - 1}; }
...
#define FMT_STATIC_FORMAT(fmt_str, ...)                            \
  conduit_fmt::static_format_result<                                       \
