...
FMT_FUNC void format_error_code(detail::buffer<char>& out, int error_code,
  ...
  if (message.size() <= inline_buffer_size - error_code_size)
    conduit_fmt::format_to(it, FMT_STRING("{}{}"), message, SEP);
  conduit_fmt::format_to(it, FMT_STRING("{}{}"), ERROR_STR, error_code);
...
template <> struct formatter<detail::bigint> {
  ...
  auto format(const detail::bigint& n, format_context& ctx) const
    ...
    for (auto i = n.bigits_.size(); i > 0; --i) {
      ...
      if (first) {
        out = conduit_fmt::format_to(out, FMT_STRING("{:x}"), value);
        first = false;
        continue;
      }
      out = conduit_fmt::format_to(out, FMT_STRING("{:08x}"), value);
    }
    if (n.exp_ > 0)
      out = conduit_fmt::format_to(out, FMT_STRING("p{}"),
