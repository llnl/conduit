...
FMT_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
  ...
  if (detail::const_check(F::is_signed && !T::is_signed)) {
    ...
    if (conduit_fmt::detail::is_negative(from)) {
      ec = 1;
      return {};
    }
...
inline auto gmtime(std::time_t time) -> std::tm {
  struct dispatcher {
    ...
    inline auto run() -> bool {
      using namespace conduit_fmt::detail;
...
void write_floating_seconds(memory_buffer& buf, Duration duration,
  ...
  conduit_fmt::format_to(std::back_inserter(buf), FMT_STRING("{:.{}f}"),
...
class year_month_day {
 private:
  conduit_fmt::year year_;
  conduit_fmt::month month_;
  conduit_fmt::day day_;

 public:
  year_month_day() = default;
  constexpr year_month_day(const year& y, const month& m, const day& d) noexcept
      : year_(y), month_(m), day_(d) {}
  constexpr auto year() const noexcept -> conduit_fmt::year { return year_; }
  constexpr auto month() const noexcept -> conduit_fmt::month { return month_; }
  constexpr auto day() const noexcept -> conduit_fmt::day { return day_; }
};
