...
//-----------------------------------------------------------------------------
TEST(conduit_relay_io_silo, check_topo_is_correct)
{
    ...
    conduit::blueprint::mesh::topology::uniform::to_unstructured(mesh["topologies"].child(0), topo_dst, coords_dst);

    std::cout << "TOPO_DST" << std::endl;
    std::cout << topo_dst.to_yaml() << std::endl;

    std::cout << "COORDS_DST" << std::endl;
    std::cout << coords_dst.to_yaml() << std::endl;

    mesh["topologies/mesh"].set(topo_dst);
    mesh["coordsets/coords"].set(coords_dst);
    // mesh["topologies/"]
    ...
    // conduit::relay::io::blueprint::save_mesh(mesh, "andres-justin-adventure", "hdf5");
}
