...
//-----------------------------------------------------------------------------
TEST(conduit_execution, direct_array_reader_writer)
{
    const index_t size = 8;
    float64 src[size];
    float64 dst[size];
    for(index_t i = 0; i < size; i++)
    {
        src[i] = static_cast<float64>(i + 1);
        dst[i] = 0.0;
    }

    conduit::execution::DirectArrayReader<float64> reader{src};
    conduit::execution::DirectArrayWriter<float64> writer{dst};

    for(index_t i = 0; i < size; i++)
    {
        EXPECT_EQ(reader[i], static_cast<float64>(i + 1));
        writer.set(i, 2.0 * reader[i]);
    }

    for(index_t i = 0; i < size; i++)
    {
        EXPECT_EQ(dst[i], 2.0 * static_cast<float64>(i + 1));
    }
}
                   ...
                   bool &dst_is_direct)
{
    ...
    // NOTE: these generic lambdas only forward, the device lambda lives in the
    // named template kernel (nvcc constraint).
    conduit::execution::dispatch_array_read_and_write(src_acc, dst_acc,
                                                      [&](auto src, auto dst)
    {
...
//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_native_path)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;
    const std::vector<float64> src_vals = make_dispatch_src_vals<float64>(size);
    const std::vector<float64> dst_vals = make_execution_des_vals(size);
    ...
    // contiguous, matching dtype -> native pointers are available
    EXPECT_TRUE(src_acc.packed_ptr() != nullptr);
    EXPECT_TRUE(dst_acc.packed_ptr() != nullptr);
    ...
    // both dispatches must have taken the raw pointer branch
    EXPECT_TRUE(src_is_direct);
    EXPECT_TRUE(dst_is_direct);

    float64_accessor res_acc(node["des"]);
    res_acc.use_with(ExecutionPolicy::host());
    EXPECT_EQ(res_acc.number_of_elements(), size);
    for(index_t i = 0; i < size; i++)
    {
        EXPECT_EQ(res_acc[i], 2.0 * static_cast<float64>(i + 1));
    }
}

//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_fallback_dtype_mismatch)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;
    const std::vector<float32> src_vals = make_dispatch_src_vals<float32>(size);
    const std::vector<int32>   dst_vals(static_cast<size_t>(size), 0);
    ...
    // dtypes do not match float64 -> no native pointer
    EXPECT_TRUE(src_acc.packed_ptr() == nullptr);
    EXPECT_TRUE(dst_acc.packed_ptr() == nullptr);

    ExecutionPolicy policy = ExecutionPolicy::serial();
    bool src_is_direct = true;
    bool dst_is_direct = true;
    run_dispatch_scale(policy, src_acc, dst_acc, src_is_direct, dst_is_direct);

    // both dispatches must have taken the accessor branch
    EXPECT_FALSE(src_is_direct);
    EXPECT_FALSE(dst_is_direct);

    // values are converted on the fly, results are still correct
    float64_accessor res_acc(node["des"]);
    res_acc.use_with(ExecutionPolicy::host());
    EXPECT_EQ(res_acc.number_of_elements(), size);
    for(index_t i = 0; i < size; i++)
...
//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_fallback_strided)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;

    // a buffer holding 'size' values of interest interleaved with junk
    std::vector<float64> src_buffer(static_cast<size_t>(2 * size), -1.0);
    std::vector<float64> dst_buffer(static_cast<size_t>(2 * size), -1.0);
    for(index_t i = 0; i < size; i++)
    {
        src_buffer[static_cast<size_t>(2 * i)] = static_cast<float64>(i + 1);
    }

    // every other element, so the accessor is strided (not densely packed)
    const index_t stride = 2 * static_cast<index_t>(sizeof(float64));
    float64_accessor src_acc(src_buffer.data(), DataType::float64(size, 0, stride));
    float64_accessor dst_acc(dst_buffer.data(), DataType::float64(size, 0, stride));
    ...
    for(index_t i = 0; i < size; i++)
    {
        // strided elements hold the result
        EXPECT_EQ(dst_buffer[static_cast<size_t>(2 * i)],
                  2.0 * static_cast<float64>(i + 1));
        // the interleaved elements were left alone
        EXPECT_EQ(dst_buffer[static_cast<size_t>(2 * i + 1)], -1.0);
    }
}

//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_offset_but_packed)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;
    const index_t skip = 3;
    const index_t offset = skip * static_cast<index_t>(sizeof(float64));

    // buffers with 'skip' leading elements that are not part of the accessors
    std::vector<float64> src_buffer(static_cast<size_t>(size + skip), -1.0);
    std::vector<float64> dst_buffer(static_cast<size_t>(size + skip), -1.0);
    for(index_t i = 0; i < size; i++)
    {
        src_buffer[static_cast<size_t>(skip + i)] = static_cast<float64>(i + 1);
    }

    // densely packed float64, but starting at a non zero offset
    float64_accessor src_acc(src_buffer.data(), DataType::float64(size, offset));
    float64_accessor dst_acc(dst_buffer.data(), DataType::float64(size, offset));

    // packed layout -> native pointers are available, shifted by the offset
    EXPECT_TRUE(src_acc.packed_ptr() != nullptr);
    EXPECT_TRUE(dst_acc.packed_ptr() != nullptr);
    EXPECT_EQ(src_acc.packed_ptr(), src_buffer.data() + skip);
    EXPECT_EQ(dst_acc.packed_ptr(), dst_buffer.data() + skip);
    ...
    {
        // the elements before the offset were left alone
        EXPECT_EQ(dst_buffer[static_cast<size_t>(i)], -1.0);
    }

    for(index_t i = 0; i < size; i++)
    {
        // the results land at the offset elements
        EXPECT_EQ(dst_buffer[static_cast<size_t>(skip + i)],
                  2.0 * static_cast<float64>(i + 1));
    }
}

//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_nested_combinations)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;

    for(int src_native = 0; src_native < 2; src_native++)
    {
        for(int dst_native = 0; dst_native < 2; dst_native++)
        {
            CONDUIT_INFO("dispatch_array_nested_combinations:"
                         << " src_native=" << src_native
                         << " dst_native=" << dst_native);
            ...
            if(dst_native)
            {
                node["des"].set(std::vector<float64>(static_cast<size_t>(size), 0.0));
            }
            else
            {
            ...
            bool src_is_direct = false;
            bool dst_is_direct = false;
            run_dispatch_scale(policy, src_acc, dst_acc,
                               src_is_direct, dst_is_direct);

            EXPECT_EQ(src_is_direct, src_native == 1);
            EXPECT_EQ(dst_is_direct, dst_native == 1);
                      ...
                      bool &is_direct)
{
    ...
    // NOTE: this generic lambda only forwards, the device lambda lives in the
    // named template kernel (nvcc constraint).
    conduit::execution::dispatch_array_write(acc, [&](auto vals)
    {
...
//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_integer_branch_selection)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;

    // native path: int64 data through an int64_accessor
    {
        Node node;
        node["conn"].set(std::vector<int64>(static_cast<size_t>(size), 0));

        int64_accessor conn_acc(node["conn"]);
        EXPECT_TRUE(conn_acc.packed_ptr() != nullptr);

        ExecutionPolicy policy = ExecutionPolicy::serial();
        bool is_direct = false;
        run_dispatch_int_fill(policy, conn_acc, is_direct);

        EXPECT_TRUE(is_direct);

        int64_accessor res_acc(node["conn"]);
        res_acc.use_with(ExecutionPolicy::host());
        EXPECT_EQ(res_acc.number_of_elements(), size);
        for(index_t i = 0; i < size; i++)
        {
            EXPECT_EQ(res_acc[i], static_cast<int64>(2 * (i + 1)));
        }
    }

    // fallback path: int32 data through an int64_accessor
    {
        Node node;
        node["conn"].set(std::vector<int32>(static_cast<size_t>(size), 0));
    ...
    // native path for the index_t alias: the size + signedness based type
    // matching must select the direct branch here too
    {
        Node node;
        node["conn"].set(std::vector<index_t>(static_cast<size_t>(size), 0));

        index_t_accessor conn_acc(node["conn"]);
        EXPECT_TRUE(conn_acc.packed_ptr() != nullptr);
        ...
        index_t_accessor res_acc(node["conn"]);
        res_acc.use_with(ExecutionPolicy::host());
        EXPECT_EQ(res_acc.number_of_elements(), size);
        for(index_t i = 0; i < size; i++)
        {
            EXPECT_EQ(res_acc[i], static_cast<index_t>(2 * (i + 1)));
        }
    }
}
                     ...
                     bool &is_direct)
{
    ...
    // NOTE: this generic lambda only forwards, the device lambda lives in the
    // named template kernel (nvcc constraint).
    conduit::execution::dispatch_array_read_write(acc, [&](auto vals)
    {
        is_direct = is_direct_array(vals);

        run_dispatch_inplace_kernel(policy, size, vals);
    });
}

//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_read_write_in_place)
{
    conduit_device_prepare();

    const index_t size = EXECUTION_TEST_ARRAY_SIZE;

    // native path: float64 data through a float64_accessor
    {
        Node node;
        node["vals"].set(make_dispatch_src_vals<float64>(size));

        float64_accessor vals_acc(node["vals"]);
        EXPECT_TRUE(vals_acc.packed_ptr() != nullptr);

        ExecutionPolicy policy = ExecutionPolicy::serial();
        bool is_direct = false;
        run_dispatch_inplace(policy, vals_acc, is_direct);

        EXPECT_TRUE(is_direct);

        float64_accessor res_acc(node["vals"]);
        res_acc.use_with(ExecutionPolicy::host());
        EXPECT_EQ(res_acc.number_of_elements(), size);
        for(index_t i = 0; i < size; i++)
        {
    ...
    // fallback path: float32 data through a float64_accessor
    {
        Node node;
        node["vals"].set(make_dispatch_src_vals<float32>(size));
...
//-----------------------------------------------------------------------------
void
run_test_dispatch_array_policies()
{
    conduit_device_prepare();
    ...
    for_each_enabled_policy([&](ExecutionPolicy policy)
    {
        CONDUIT_INFO("dispatch_array policy=" << policy.policy_name());

        // native path: float64 data
        {
            Node node;
            node["src"].set(make_dispatch_src_vals<float64>(size));
        ...
        // fallback path: float32 src, int32 dst
        {
            Node node;
            node["src"].set(make_dispatch_src_vals<float32>(size));
            node["des"].set(std::vector<int32>(static_cast<size_t>(size), 0));
...
//-----------------------------------------------------------------------------
TEST(conduit_execution, dispatch_array_policies)
{
    // this is a separate func to avoid issue with lambda vs gtest macro
    run_test_dispatch_array_policies();
}
